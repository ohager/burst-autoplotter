...
const selectIsAVX = select(state => state.instructionSet && state.instructionSet.indexOf('AVX') !== -1);

const selectInstructionSet = select(state => state.instructionSet);

const selectOutputPath = select(state => state.outputPath);

const selectUsedThreads = select(state => state.usedThreads);

const selectUsedMemory = select(state => state.usedMemory);

const selectAccount = select(state => state.account);

const selectTotalWrittenNonces = select(state => state.totalWrittenNonces);

const selectTotalNonces = select(state => state.totalNonces);

const selectCacheFile = select(state => state.cacheFile);

const selectPlotCount = select(state => state.plotCount);

const selectCurrentPlotIndex = select(state => state.currentPlot.index);

const selectTotalRemainingNonces = select(state => state.totalNonces - state.totalWrittenNonces);

const selectElapsedTimeInSecs = select(state => Math.floor((Date.now() - state.startTime) / 1000));

const selectCurrentPlotNonces = select(state => state.currentPlot.nonces);

const selectCurrentPlotWrittenNonces = select(state => state.currentPlot.writtenNonces);

const selectCurrentPlotRemainingNonces = select(state => state.currentPlot.nonces - state.currentPlot.writtenNonces);

const selectEffectiveNoncesPerSeconds = select(state => {
...
const selectTotalEstimatedDurationInSecs = select(state => {
	const npm = selectEffectiveNoncesPerSeconds();
	return npm ? Math.floor(selectTotalRemainingNonces() / npm) : null;
});

const selectCurrentPlotEstimatedDurationInSecs = select(state => {
	const npm = selectEffectiveNoncesPerSeconds();
	return npm ? Math.floor(selectCurrentPlotRemainingNonces() / npm) : null;
});

const selectScoopPercentage = select(state => state.scoopPercentage || 0);
